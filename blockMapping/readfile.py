...
def extract_data_from_file(filename, file_type, **kwargs):
    ...
    df.to_pickle("blockMapping/JSON/pickle.pkl")
    t = time.localtime()
    current_time = time.strftime("%H:%M:%S", t)
...
def process_data(list_diseases):
    df = pd.read_pickle("blockMapping/JSON/pickle.pkl")
    df["Combo"] = np.where(df[list_diseases].sum(axis = 1, skipna =True)>1, 1, np.nan)
    diseaseList = df.columns[10:]
