...
SELECT
	...
	,patient_id
	, CASE WHEN MAX(fa_risk) = 'High' THEN true ELSE false END AS fa_risk_status
    , CASE WHEN MAX(fa) = 'Yes' THEN TRUE ELSE FALSE END AS fa_status
FROM """ + DATABASE_NAME + """.ulm_fa_risk as fa
INNER JOIN """ + DATABASE_NAME + """.ulm_file as f
	ON fa.file_id = f.id
WHERE
...
SELECT
    ...
    ,patient_id
	,CASE
		WHEN MAX(LOWER(latest_outcome)) IN ('admit', 'accept/admit', 'admit as existing case') AND application_status = 'CASE CLOSED' THEN 'Admitted'
        WHEN MAX(LOWER(latest_outcome)) IN ('pre assign withdrawn', 'pre assign withdrawn – bpr', 'reject by agency', 'withdrawal after revert back by agency', 'Withdrawn', 'withdrawn & case closed by referral source') AND application_status = 'CASE CLOSED' THEN 'Withdrawn'
        ELSE 'Undetermined'
	END AS irms_status
FROM """ + DATABASE_NAME + """.ulm_irms as irms
INNER JOIN """ + DATABASE_NAME + """.ulm_file as f
...
WHERE
    ...
    AND pat_id.region = %s
    AND NOT(fa_risk_status = False AND fa_risk_status = False AND tcu_status = False AND irms_status != 'Admitted')
...
SELECT
 pat_id.house_floor
 ,pat_id.house_unit
 ,MAX(fa_risk_status) as fa_risk_status
 ,MAX(fa_status) as fa_status
 ,MAX(tcu_status) as tcu_status
 ,MAX(irms_status) as irms_status
 ,MAX(rental_status) as rental_status
...
AND street = %s
AND NOT(fa_risk_status = False AND fa_risk_status = False AND tcu_status = False AND irms_status != 'Admitted')
GROUP BY pat_id.house_floor, pat_id.house_unit
"""

#LEFT JOIN """ + POSTAL_CODE + \
#"""	ON pat_id.postal_code >= postal_code.low_bound
#    AND pat_id.postal_code <= postal_code.high_bound